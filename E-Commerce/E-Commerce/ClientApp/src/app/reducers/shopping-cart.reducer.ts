...
export function shoppingCartReducer(
  state: ShoppingCartModel = initial,
  action: ShoppingStateActions.Actions
) {
  switch (action.type) {
    ...
    case ShoppingStateActions.REMOVE_SHOPPING_CART_ITEM:
      let newShoppingCartItems = state.shoppingCartItems.filter(
    ...
    case ShoppingStateActions.ADD_SHOPPING_CART_ITEM:
      if (state.shoppingCartItems.length > 0) {
        let existing = state.shoppingCartItems.filter(
    ...
    case ShoppingStateActions.CHANGE_QUANTITY:
      let deleted = state.shoppingCartItems.filter(
