...
export class OrderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
      .get<ShippingCompanyModel[]>(this.url + 'ShippingCompany')
      .subscribe((response) => (this.shippingList = response));
  ...
  submit() {
    ...
    this.http.post(this.url + 'Order', model).subscribe((response) =>
      this.shoppingCartService.activeShoppingCard().subscribe((response) => {
        if (response) {
          this.store.dispatch(
            new ShoppingCartActions.SetShoppingCart(response)
          );
        }
      })
    );
  ...
  putUserInfoForm() {
    if (this.userInfoForm.touched) {
      ...
      this.authService.putUserInformations(model).subscribe();
    }
  }
}
